...
#' @param taxobj Taxonomy summary objects generated by \code{\link{tax_summary}}.
...
#' @param palette Character. Palette for visualization,default:"Spectral",recommended to use "Paired" for more than 15 tax.See supported palette in \code{\link{scale_colour_brewer}}
#' @param nrow Numeric. Number of rows when wrap panels,default:NULL. See details in \code{\link{facet_wrap}}
...
community_plot=function(taxobj,taxlevel,n=10,palette="Spectral",nrow=NULL,rmprefix=NULL){
  ...
  long_topframe<-data.frame(topframe[,-1],row.names = topframe[,1]) %>%
    combine_and_translate(groupframe,"tax","rel_abundance",T)
  ...
  plot=ggplot(long_topframe, aes(x=long_topframe[,rep_location], y = long_topframe[,'rel_abundance'], fill = long_topframe[,'tax'] )) +
    geom_col(position = 'stack', width = 0.8)+ ##采用stack：堆积模式##
    scale_fill_manual(values = getPalette(nrow(topframe)))+
    scale_y_continuous(labels = scales::percent,expand = c(0,0)) +  ##转百分比坐标，剔除空隙##
