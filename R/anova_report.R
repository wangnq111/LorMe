...
#' }
anova_report=function(data,treatment_col,value_col,prior=F,comparison_method="Auto",equally_rep=T){
  ...
  cat("\n\n")
  if(as.numeric(aov_frame[1,5])<0.05|prior==T){
    ...
    if(comparison_method %in% c("LSD","bonferroni")){
      ...
      SCFresults=scheffe.test(aovmodel,trt="treatment",console = F)
      cat("###Student-Newman-Keuls statistics#### \n\n")
