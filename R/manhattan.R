...
manhattan=function(inputframe,taxlevel="Phylum",control_name,mode="all",top_n=NULL,palette="Set1",select_tax=NULL,rmprefix=NULL){
  ...
  p_width=ggplot(inputframe_select, aes(x=inputframe_select[,taxlevel], y=-log10(inputframe_select[,'padj']))) +  #
    ...
    geom_point(aes(color=inputframe_select[,taxlevel],shape=inputframe_select[,'tag']),alpha=.8,position=position_jitter(0.5)) + #,stroke=2#绘制散点, position调整离散程度，stroke参数调整点的路径的粗细
    scale_shape_manual(values=aes_shape)+
    #scale_size_manual(values=c("IIII"=2,"III"=1.5,"II"=1,"I"=0.5))+
    labs(x=NULL, y="-log10(q value)",shape="")+#删掉x轴标签
  ...
  p_circle=ggplot(inputframe_select, aes(x=inputframe_select[,taxlevel], y=log10(inputframe_select[,'padj']))) +  #
    ...
    geom_hline(yintercept=log10(0.05), linetype=2, color="lightgrey") +# 添加显著阈值线
    geom_hline(yintercept=log10(0.001), linetype=2, color="lightgrey") +# 添加显著阈值线
    geom_hline(yintercept=log10(1e-5), linetype=2, color="lightgrey") +# 添加显著阈值线
    geom_point(aes(color=inputframe_select[,taxlevel],shape=inputframe_select[,'tag']),alpha=.8,position=position_jitter(0.5)) + #,stroke=2#绘制散点, position调整离散程度，stroke参数调整点的路径的粗细
    scale_shape_manual(values=aes_shape)+
    #scale_size_manual(values=c("IIII"=2,"III"=1.5,"II"=1,"I"=0.5))+
    scale_y_continuous(limits = c(1.5*max_y,0.5))+
    labs(x=NULL, y="",shape="")+#删掉x轴标签
    theme_zg()+
    ...
    theme(#legend.position="top", #图例置顶
      ...
      axis.text.x = element_text())#x轴坐标标签倾斜
