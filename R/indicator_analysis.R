...
#' @param func Default: "r.g".The function to use for the indicator analysis, see in \code{\link{multipatt}}
...
#' @examples
#' data("Two_group")
#' indicator_results=indicator_analysis(taxobj = Two_group,taxlevel = "Genus")
...
indicator_analysis=function(taxobj,taxlevel,func="r.g",reads=F){
  ...
  cat("Calculating.......\n")
  set.seed(999)
  indicator_results=multipatt(as.data.frame(t(inputframe)),condition,func = func,control=how(nperm=1000)) %$%
    as.data.frame(sign)
