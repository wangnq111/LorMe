...
wilcox_test_report=function(data,treatment_col,value_col,paired,subject_col){
  ...
  if(paired==T){
    ...
    wilcox_results=wilcox.test(rank1,rank2,paired = T,correct = F,exact = F)
    ...
    wilcox_results=wilcox.test(get(colnames(data)[value_col])~get(colnames(data)[treatment_col]),data=data,correct = F,exact = F)
    p_exact=wilcox_results$p.value
