...
#'
#' @note
#' Package \code{\link{coin}} is required in this function.
#' @export
#'
...
#'   print(wilcox_result)
#' }
wilcox_test_report=function(data,treatment_col,value_col,paired,subject_col){
  ...
  if(paired==T){
    ...
    p_exact=wilcox.test(rank1,rank2,paired = T,correct = F,exact = F)$p.value
    p_estimate=wilcox.test(rank1,rank2,paired = T,correct = T,exact = F)$p.value
    T1=wilcox.test(rank1,rank2,paired = T,correct = F,exact = F)$statistic
    T2=wilcox.test(rank2,rank1,paired = T,correct = F,exact = F)$statistic
    cat("###Statistics#### \n\n")
    ...
    cat("###Conclusion####\n")
    if(p_exact>0.05){cat("The change that occurred with the treatment is greater than would be expected by chance; there is a statistically significant difference  (P = ",p_exact,").\n\n")
      }else{cat("The change that occurred with the treatment is not great enough to exclude the possibility that it is due to chance  (P =",p_exact,").\n\n")}
  }else{
    cat("###Mann-Whitney Rank Sum Test#### \n\n")
    p_exact=wilcox.test(get(colnames(data)[value_col])~get(colnames(data)[treatment_col]),data=data,correct = F,exact = F)$p.value
    p_estimate=wilcox.test(get(colnames(data)[value_col])~get(colnames(data)[treatment_col]),data=data,correct = T,exact = F)$p.value
    U=wilcox.test(get(colnames(data)[value_col])~get(colnames(data)[treatment_col]),data=data,correct = F,exact = F)$statistic %>% as.numeric()
    cat("###Statistics#### \n\n")
    cat("Mann-Whitney U Statistic= ",U," \n\n")
    cat("n(small)= ",min(basic_data$N),", n(big)= ",max(basic_data$N),"\n")
    cat("P_estimate = ",p_estimate," ,P_exact = ",p_exact,"\n\n")
    cat("###Conclusion####\n")
    if(p_exact<0.05){cat("The difference in the median values between the two groups is greater than would be expected by chance; there is a statistically significant difference  (P = ",p_exact,").\n\n")
    }else{cat("The difference in the median values between the two groups is not great enough to exclude the possibility that the difference is due to random sampling variability; there is not a statistically significant difference  (P = ",p_exact,").\n\n")}
    }
